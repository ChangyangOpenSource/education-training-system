	...
	<view class="">
		...
		<view class="" v-if="iosASS==0">
			<view class="flex" v-for="(item,index) in topicList.questionPaperOption" :key='index'
				...
				<view class="">
					<view :class="{theAnswer:true}" v-if="item.subType==1">
						{{item.content}}
					</view>
		...
		methods: {
			onIput(item, index) {
				if (this.iosASS == 2) {
					...
					console.log(aaa, 'h5 判断提交1')
					var thatPage = aaa[aaa.length - 1];
					thatPage.onJudgment(item, index, 'chu', 'xia')
					// this.$parent.$parent.$parent.onJudgment(item, index, 'chu', 'xia')
					//#endif
					//#ifdef MP-WEIXIN
					this.$parent.onJudgment(item, index, 'chu', 'xia')
					//#endif
				} else {
					//#ifdef H5
					var aaa = getCurrentPages();
					console.log(aaa, 'h5 判断提交2')
					var thatPage = aaa[aaa.length - 1];
					thatPage.onJudgment(item, index)
					// this.$parent.$parent.$parent.onJudgment(item, index)
					//#endif
					//#ifdef MP-WEIXIN
					this.$parent.onJudgment(item, index)
					//#endif
				}
			},
